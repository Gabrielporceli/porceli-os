...
export function QuickAddMenu({ x, y, onPick, onClose }: QuickAddMenuProps) {
  ...
  return (
    <>
      ...
      <div
        className="liquid-glass fixed z-50 flex w-56 flex-col rounded-2xl text-white"
        style={{ left, top }}
      >
        ...
        <div className="max-h-72 overflow-y-auto p-1.5">
          {results.length === 0 && (
            <p className="px-2 py-4 text-center text-[11px] text-white/40">Nada encontrado</p>
          )}
          {results.map((v) => (
            <button
              key={`${v.category}-${v.id}`}
            ...
            >
              ...
              <span className="min-w-0 flex-1">
                <span className="block truncate text-xs font-medium text-white">{v.label}</span>
