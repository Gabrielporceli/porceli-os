...
/** Fixed right-hand settings panel for the selected element. Replaces the old
...
export function PropertiesPanel({ node, onUpdate, onDelete }: PropertiesPanelProps) {
  ...
  return (
    <aside className="flex w-72 shrink-0 flex-col overflow-y-auto border-l border-porceli-gray-800 bg-porceli-gray-900/70">
      <div className="border-b border-porceli-gray-800 px-4 py-3">
        <p className="text-[10px] font-semibold uppercase tracking-wide" style={{ color: def.color }}>
          {def.label}
        </p>
        <p className="text-sm font-semibold text-porceli-gray-100">{data.label || variant.label}</p>
      </div>
      ...
      <div className="flex flex-col gap-3 px-4 py-3 text-xs">
        <Field label="Nome">
          <input
            ...
            className="w-full rounded-md bg-porceli-gray-800 px-2 py-1.5 text-porceli-gray-100 outline-none focus:ring-1 focus:ring-porceli-purpleLight"
          />
        </Field>
        ...
        {category === 'traffic' && (
          <Field label="Visitas / mês">
            <input
              ...
              className="w-full rounded-md bg-porceli-gray-800 px-2 py-1.5 text-right text-porceli-gray-100 outline-none focus:ring-1 focus:ring-porceli-purpleLight"
            />
          </Field>
        )}
        ...
        <Field label="Anotações">
          <textarea
            ...
            rows={3}
            className="w-full resize-none rounded-md bg-porceli-gray-800 px-2 py-1.5 text-porceli-gray-100 outline-none focus:ring-1 focus:ring-porceli-purpleLight"
          />
        </Field>
        ...
        {/* Computed summary */}
        <div className="rounded-lg border border-porceli-gray-800 bg-porceli-gray-950/60 p-3">
          <p className="mb-2 text-[10px] font-semibold uppercase tracking-wide text-porceli-gray-500">Resultado simulado</p>
        ...
        <button
          ...
          className="flex items-center justify-center gap-1.5 rounded-md border border-red-900/40 py-1.5 text-red-400 hover:bg-red-950"
        >
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <label className="flex flex-col gap-1 text-porceli-gray-400">
...
function Stat({ label, value, accent }: { label: string; value: string; accent?: string }) {
  return (
    <div className="flex items-center justify-between py-0.5">
      <span className="text-[11px] text-porceli-gray-500">{label}</span>
      <span className={`text-xs font-bold tabular-nums ${accent ?? 'text-porceli-gray-100'}`}>{value}</span>
    </div>
  );
}
