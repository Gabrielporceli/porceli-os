-- ============================================================================
-- FIX: Adicionar parâmetros p_is_group e p_contact_photo à função process_webhook_message
-- O webhook da Evolution estava passando esses parâmetros mas a função não os aceitava,
-- causando falha na criação de leads e conversas.
-- ============================================================================
...
CREATE OR REPLACE FUNCTION public.process_webhook_message(
  ...
  p_contact_photo text DEFAULT NULL::text
)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- Para grupos, usar o remote_jid completo como identificador
  IF p_is_group THEN
    phone_clean := p_numero;
  END IF;
  ...
  -- PASSO 1: Buscar ou criar contato - APENAS se NÃO for grupo e a mensagem NÃO foi enviada pelo usuário
  ...
  -- PASSO 2: Buscar conversa pelo número limpo ou remote_jid para evitar duplicação
  SELECT conv.id INTO conversation_uuid
  FROM public.conversations conv
  WHERE conv.user_id = p_user_id
    AND (
      conv.remote_jid = p_numero
      OR conv.phone = phone_clean
      OR REGEXP_REPLACE(COALESCE(conv.remote_jid, ''), '[^0-9+]', '', 'g') = REGEXP_REPLACE(phone_clean, '[^0-9+]', '', 'g')
      OR REGEXP_REPLACE(COALESCE(conv.numero, ''), '[^0-9+]', '', 'g') = REGEXP_REPLACE(phone_clean, '[^0-9+]', '', 'g')
    )
  ...
  -- PASSO 3: Sincronizar remotejid do lead com remote_jid da conversation (apenas para não-grupos)
  IF NOT p_is_group THEN
    UPDATE public.leads
    SET remotejid = p_numero,
        updated_at = p_data_hora
    WHERE user_id = p_user_id
      AND phone = phone_clean
      AND (remotejid IS NULL OR remotejid != p_numero);
  END IF;

  -- PASSO 4: Inserir mensagem com todos os campos de mídia
